...
describe('PII Redaction', () => {
  ...
  describe('placeholder strategy', () => {
    it('replaces email with placeholder', async () => {
      const result = await redactPII('Contact john@acme.com', { strategy: 'placeholder' });
      expect(result.redactedText).toContain('[EMAIL_');
      expect(result.redactedText).not.toContain('john@acme.com');
      expect(result.detections).toHaveLength(1);
    });

    it('uses incrementing indices for same type', async () => {
      const result = await redactPII('Email a@b.com and c@d.com', { strategy: 'placeholder' });
      expect(result.redactedText).toContain('[EMAIL_1]');
      expect(result.redactedText).toContain('[EMAIL_2]');
    });

    it('replaces SSN with placeholder', async () => {
      const result = await redactPII('SSN: 123-45-6789', { strategy: 'placeholder' });
      expect(result.redactedText).toContain('[SSN_');
      expect(result.redactedText).not.toContain('123-45-6789');
    });

    it('replaces multiple PII types', async () => {
      const result = await redactPII('Email john@acme.com SSN 123-45-6789');
      expect(result.redactedText).toContain('[EMAIL_');
      expect(result.redactedText).toContain('[SSN_');
    ...
    it('is the default strategy', async () => {
      const result = await redactPII('Email john@acme.com');
      expect(result.redactedText).toContain('[EMAIL_');
    });
  });
  ...
  describe('synthetic strategy', () => {
    it('replaces email with synthetic email', async () => {
      const result = await redactPII('Contact john@acme.com', { strategy: 'synthetic' });
      expect(result.redactedText).not.toContain('john@acme.com');
      expect(result.redactedText).toContain('@example.');
    });

    it('replaces SSN with synthetic SSN', async () => {
      const result = await redactPII('SSN: 123-45-6789', { strategy: 'synthetic' });
      expect(result.redactedText).not.toContain('123-45-6789');
  ...
  describe('hash strategy', () => {
    it('replaces email with hash', async () => {
      const result = await redactPII('Contact john@acme.com', { strategy: 'hash' });
      expect(result.redactedText).not.toContain('john@acme.com');
      expect(result.redactedText.length).toBeGreaterThan(0);
    });

    it('same input produces same hash (consistent)', async () => {
      const r1 = await redactPII('john@acme.com', { strategy: 'hash' });
      const r2 = await redactPII('john@acme.com', { strategy: 'hash' });
      expect(r1.redactedText).toBe(r2.redactedText);
    });

    it('different inputs produce different hashes', async () => {
      const r1 = await redactPII('a@b.com', { strategy: 'hash' });
      const r2 = await redactPII('c@d.com', { strategy: 'hash' });
  ...
  describe('deRedact', () => {
    it('restores original values from placeholder', async () => {
      const result = await redactPII('Email john@acme.com', { strategy: 'placeholder' });
      const restored = deRedact(result.redactedText, result.mapping);
      expect(restored).toBe('Email john@acme.com');
    });

    it('restores original values from synthetic', async () => {
      const result = await redactPII('Email john@acme.com', { strategy: 'synthetic' });
      const restored = deRedact(result.redactedText, result.mapping);
      expect(restored).toBe('Email john@acme.com');
    });

    it('restores multiple PII values', async () => {
      const original = 'Email john@acme.com SSN 123-45-6789';
      const result = await redactPII(original, { strategy: 'placeholder' });
      const restored = deRedact(result.redactedText, result.mapping);
  ...
  describe('mapping', () => {
    it('maps replacement to original', async () => {
      const result = await redactPII('john@acme.com');
    ...
    it('empty for no PII', async () => {
      const result = await redactPII('Hello world');
  ...
  describe('edge cases', () => {
    ...
    it('type filtering works', async () => {
      ...
      const result = await redactPII(text, { types: ['email'] });
  ...
  describe('mask strategy', () => {
    it('masks credit card showing last 4 digits', async () => {
      const result = await redactPII('Card: 4111-1111-1111-1111', { strategy: 'mask' });
    ...
    it('masks email keeping first char and domain', async () => {
      const result = await redactPII('Email: john@acme.com', { strategy: 'mask' });
      expect(result.redactedText).toContain('j***@acme.com');
      expect(result.redactedText).not.toContain('john@acme.com');
    });

    it('masks phone showing last 4 digits', async () => {
      const result = await redactPII('Call (555) 123-4567', { strategy: 'mask' });
      expect(result.redactedText).toContain('***-***-4567');
      expect(result.redactedText).not.toContain('(555) 123-4567');
    });

    it('masks SSN showing last 4 digits', async () => {
      const result = await redactPII('SSN: 123-45-6789', { strategy: 'mask' });
    ...
    it('uses custom mask character', async () => {
      const result = await redactPII('SSN: 123-45-6789', {
        strategy: 'mask',
        masking: { char: '#' },
      });
      expect(result.redactedText).toContain('###-##-6789');
    ...
    it('populates mapping for de-redaction', async () => {
      const result = await redactPII('Email: john@acme.com', { strategy: 'mask' });
      expect(result.mapping.size).toBe(1);
      const [replacement, original] = [...result.mapping.entries()][0];
      expect(original).toBe('john@acme.com');
  ...
  describe('providers', () => {
    it('merges detections from additional providers', async () => {
      ...
      const result = await redactPII(text, { providers: [mockProvider] });
    ...
    it('gracefully handles provider errors', async () => {
      ...
      const result = await redactPII('john@acme.com', { providers: [failingProvider] });
